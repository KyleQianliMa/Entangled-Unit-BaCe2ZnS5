...
def p(u1,th1):
    return np.cos(u1)*np.matrix([1,0,0,0])+np.exp(1j*th1)*np.sin(u1)*np.matrix([0,0,0,1])
...
E=1e5
for hscale in np.arange(0.0,4.2,0.01):
    h=hscale
    for i in range(0,10):
        if h < 2:
            s1=60.0  *np.pi/180.0
            t1=120.0 *np.pi/180.0
            s2=-120.0   *np.pi/180.0
            t2=-0.0  *np.pi/180.0
            s3=0.0   *np.pi/180.0
            t3=180.0 *np.pi/180.0
            s4=0
            t4=np.pi
        else:
            s1=0.0   *np.pi/180.0
            t1=180.0 *np.pi/180.0
            s2=0.0   *np.pi/180.0
            t2=180.0 *np.pi/180.0
            s3=0.0   *np.pi/180.0
            t3=180.0 *np.pi/180.0
            s4=0
            t4=np.pi
        bnds = ((s1, t1), (s2, t2), (s2, t2), (s1, t1), (s3, t3), (s3, t3), (s3, t3), (s3, t3),
                (s4, t4), (s4, t4), (s4, t4), (s4, t4), (s4, t4), (s4, t4), (s4, t4), (s4, t4))
        
        initial=(rdm.uniform(s1,t1), rdm.uniform(s2,t2), rdm.uniform(s2,t2), rdm.uniform(s1,t1), 
                 rdm.uniform(s3,t3), rdm.uniform(s3,t3), rdm.uniform(s3,t3), rdm.uniform(s3,t3),
                 rdm.uniform(s4,t4), rdm.uniform(s4,t4), rdm.uniform(s4,t4), rdm.uniform(s4,t4), 
                 rdm.uniform(s4,t4), rdm.uniform(s4,t4), rdm.uniform(s4,t4), rdm.uniform(s4,t4))
    ...
    for i in range(16):
        angles[i+1].append(new_res.x[i]/np.pi*180.0)
    ...
    stagger_m.append(( Sp4[2]+Sp5[2]+Sp11[2]+Sp14[2]-Sp3[2]-Sp6[2]-Sp12[2]-Sp13[2]).item().real)
    # bulk_m.append(( Sp4[2]+Sp5[2]+Sp11[2]+Sp14[2]+Sp3[2]+Sp6[2]+Sp12[2]+Sp13[2]).item().real)
...
# Tpt=11 #The Tpt-th data point where transition happens
# for i in range(Tpt):
#     for j in (3,6,12,13):
...
ax[0,3].plot(field,Spin[4],'.',field,Spin[5],'.')
ax[0,3].set_title("spin4,5")
ax[0,3].set_ylim(low,upp)

ax[1,0].plot(field,Spin[1],'.',field,Spin[15],'.')
ax[1,0].set_title("spin1,15")
ax[1,0].set_ylim(low,upp)

ax[1,1].plot(field,Spin[2],'.',field,Spin[16],'.')
ax[1,1].set_title("spin2,16")
ax[1,1].set_ylim(low,upp)

ax[1,2].plot(field,Spin[7],'.',field,Spin[9],'.')
ax[1,2].set_title("spin7,9")
ax[1,2].set_ylim(low,upp)

ax[1,3].plot(field,Spin[8],'.',field,Spin[10],'.')
ax[1,3].set_title("spin8,10")
ax[1,3].set_ylim(low,upp)

ax[2,0].plot(field,np.abs(bulk_m))
ax[2,0].set_title("Bulk magnetization")

ax[2,1].plot(field,stagger_m)
ax[2,1].set_title("Stagger magnetization")

ax[2,2].plot(field,energy,'.-')
ax[2,2].set_title("System Energy")

bm=max(np.abs(bulk_m)**2)


sm=max(np.array(stagger_m)**2)

k=bm/sm

ax[2,3].plot(field, (np.abs(bulk_m))**2,'.-',color='red')
ax[2,3].plot(field, k*(np.array(stagger_m))**2,'.-',color='blue')
ax[2,3].set_title("Overlay Stagger and Bulk Magnetization squared")
...
#         ct+=1
#%%--------Quantum Tunneling------------------
bulk_m_tunnel=np.zeros(len(field))
H_c=1.9
for i in range(len(field)):
    if field[i]>H_c:
        bulk_m_tunnel[i]=bulk_m[i]*(1-np.exp(-np.sqrt(80*0.5*muB*(field[i]-H_c))))
bm=max(np.abs(bulk_m_tunnel)**2)
k=bm/sm
plt.plot(field,np.abs(bulk_m_tunnel)**2,field,k*(np.array(stagger_m))**2)